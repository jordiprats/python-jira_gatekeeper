...
class Login(QtWidgets.QDialog):
    def __init__(self, parent=None):
        global jira_url, jira_username, jira_password, debug, check_unattended_incidents, check_assigned_incidents, window_mode
        ...
        self.textInstance.setText(self.settings.value("jira_url"))
        self.textName.setText(self.settings.value("jira_username"))
        self.textPass.setText(self.settings.value("jira_password"))
        self.textTeam.setText(self.settings.value("jira_project"))
    ...
    def handleLogin(self):
        global jira_url, jira_username, jira_password, jira_project, debug, window_mode
        jira_url = self.textInstance.text()
        jira_username = self.textName.text()
        jira_password = self.textPass.text()
        jira_project = self.textTeam.text()
        ...
        self.settings.setValue("jira_url", jira_url)
        self.settings.setValue("jira_username", jira_username)
        self.settings.setValue("jira_password", jira_password)
        self.settings.setValue("jira_project", jira_project)
...
class jiraWorker(QRunnable):
    ...
    #     global jira_url, jira_username, jira_password, jira_project, debug
    #     if debug:
    #         print("    def getAssignedIncidentCount(self):")
    #     c = pysnow.client.Client(instance=jira_url, user=jira_username, password=jira_password)
    ...
    def getUnattendedIncidentCount(self, minutes=0):
        ...
        jql_request = 'project = INFRA AND status NOT IN (Backlog, Closed, Resolved, Rejected) AND assignee is EMPTY AND type = "Service Request"'
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if debug:
                        print("jiraWorker::run - unattended_incidents_minutes: "+str(e))
                ...
                print("Exception jiraWorker::run: "+str(e))
